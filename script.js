 ...
 *
 * VERSÃO 2.2 - Diagnóstico de Erros de Conexão
*/
...
async function initializeApp() {
    ...
    try {
        ...
        alert(`Não foi possível carregar os dados. Detalhes: ${error.message}. Verifique o console do desenvolvedor (F12) para mais informações e confirme as permissões da planilha.`);
    }
}
...
// --- LÓGICA DE DADOS (GOOGLE SHEETS com diagnóstico de erro melhorado) ---
async function fetchGoogleSheet(url) {
    try {
        const response = await fetch(url);
        if (!response.ok) {
            console.error('A resposta da API do Google Sheet não foi bem-sucedida.', { url: url, status: response.status, statusText: response.statusText });
            throw new Error(`Erro de HTTP ${response.status}: ${response.statusText}.`);
        }
        const text = await response.text();
        try {
            return JSON.parse(text);
        } catch (e) {
            console.error("A resposta da planilha não é um JSON válido. Provavelmente é uma página de erro do Google.", text);
            throw new Error("Formato de resposta inválido. Verifique se o script da planilha está retornando JSON.");
        }
    } catch (error) {
        // Captura erros de rede, CORS, etc.
        console.error(`Falha ao buscar dados de: ${url}`, error);
        if (error.message.includes('Failed to fetch')) {
            throw new Error('Erro de rede ou CORS. Verifique as permissões de acesso da sua planilha (deve ser "Qualquer pessoa") e o console do navegador');
        }
        throw error;
    }
}
